...
- `/home/steve/Work-space/WASHVN/.claude/skills/ba-elicitor/knowledge/bootstrap-rules.md` — Phase 0 scaffold, state init, resume detection
- `/home/steve/Work-space/WASHVN/.claude/skills/ba-elicitor/knowledge/path-resolution.md` — feature_name resolution, priority chain, action resolution
...
<path_resolution>
BEFORE any phase, resolve feature_name and action deterministically:

1. **Parse user input** for explicit `.skill-context/{name}/` path or `{name}` reference
2. **Priority chain**: explicit path > explicit name > scan .skill-context/ > derive from request
3. **Resolve action**: READ if completed | RESUME if partial | CREATE if none
4. **If tentative** (priority 4) → confirm with user before Phase 1

See `knowledge/path-resolution.md` for full decision tree.
</path_resolution>

<phase_0_bootstrap>
⚠️ RUN THIS FIRST — BEFORE PHASE 1 NORMALIZE.

1. Resolve `feature_name` via path-resolution rules
2. Scaffold `.skill-context/{feature_name}/{ba-elicitor,ba-analyst}/` 
3. Init `.skill-context/{feature_name}/_state.yaml` with `lifecycle: bootstrapping`
4. Verify scaffold exists (4 checks: root dir + 2 subdirs + state file)
5. If resume detected → skip scaffold, skip completed phases
6. If BA complete detected → READ only, refuse writes

See `knowledge/bootstrap-rules.md` for scaffold details + resume/overwrite rules.
</phase_0_bootstrap>
...
**Phase 0 — Bootstrap (path-resolution + scaffold)**
1. Run `<path_resolution>` rules → determine feature_name + action (CREATE | RESUME | READ)
2. Run `<phase_0_bootstrap>` rules → scaffold directories, init `_state.yaml`
3. If action=READ → read business-analysis.md, return summary, EXIT
4. If action=RESUME → skip completed phases, start from first incomplete phase
5. If action=CREATE → proceed to Phase 1
6. Report to parent session: `{feature_name, action, base_path, confidence, resolved_by}`
...
4. **UPDATE `_state.yaml`** → `lifecycle: normalizing`, `confidence: <N>%`, `updated_at: <ISO8601>`
5. Stop here if confidence < 60%. Emit the question list and wait for Steve's reply.
...
5. **UPDATE `_state.yaml`** → `lifecycle: eliciting`, `confidence: <N>%`, `updated_at: <ISO8601>`
...
7. **UPDATE `_state.yaml`** → `lifecycle: analyzing`, `confidence: <N>%`, `updated_at: <ISO8601>`
...
5. **UPDATE `_state.yaml`** → `lifecycle: synthesizing`, `confidence: <N>%`, `updated_at: <ISO8601>`

**Phase 5 — Handoff**
1. **UPDATE `_state.yaml`** → `lifecycle: ba-completed`, `confidence: <N>%`, `updated_at: <ISO8601>`
2. Emit a final summary to the parent session: feature_name, status, confidence, top 3 risks, top 3 [CẦN LÀM RÕ] still open, suggested next stage (Explorer).
3. Do NOT trigger Stage 0. The parent decides.
</workflow_phases>
...
- bootstrap-rules.md: Phase 0 scaffold, state init, resume/overwrite detection
- path-resolution.md: feature_name resolution, priority chain, action determination
...
- Template-style placeholder detected → CRITICAL. Rewrite without banned tokens. If you cannot, mark `[CẦN LÀM RÕ]` and stop.

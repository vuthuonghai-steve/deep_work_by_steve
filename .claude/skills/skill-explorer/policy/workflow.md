...
2.  **Khởi tạo bối cảnh**: Chạy script `python3 scripts/init_context.py {skill-name}` để khởi tạo thư mục làm việc.
...
3.  **Lưu trữ tài nguyên**:
    - Ghi nhận tất cả tài liệu nghiệp vụ, mã nguồn mẫu, API spec thu thập được vào các file độc lập bên dưới `.skill-context/{skill-name}/resources/`.
...
2.  **Biên soạn báo cáo**: Ghi toàn bộ kết quả tổng hợp vào `.skill-context/{skill-name}/exploration.md`.
3.  **Chạy kiểm định chất lượng**:
    - Tự kiểm tra bằng checklist `loop/exploration-checklist.md`.
    - Chạy bộ xác thực frontmatter: `python3 ../_shared/validators/schema_validator.py --schema ../_shared/schemas/exploration.schema.yaml .skill-context/{skill-name}/exploration.md`.

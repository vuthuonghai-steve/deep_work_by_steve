...
triggers:
  ...
  - "phân tích bằng heavy thinking"
  - "analyze X with heavy thinking"
  - "dùng heavy thinking để phân tích"
...
**Khi nào dùng Heavy Thinking (K=4-5 chains):**
- User nói "phân tích bằng heavy thinking" hoặc "dùng heavy thinking"
- Problem phức tạp, có nhiều failure modes
- Cần hiểu sâu TRƯỚC KHI thiết kế solution

**Khi dùng Heavy Thinking:**
1. Chạy K=4-5 parallel chains, mỗi chain 1 lens:
   - Chain 1: Context & State Management
   - Chain 2: Handoff & Contract Integrity
   - Chain 3: Implicit vs Explicit Assumptions
   - Chain 4: Error Handling & Silent Failures
   - Chain 5 (optional): Quality Metrics
2. Stage 2: Deliberation → synthesize → identify CORE problems
3. Đề xuất giải pháp theo CASE System (PREVENT/DETECT/RECOVER)
4. Xem `references/heavy-thinking-to-design-workflow.md` cho chi tiết

**Thực hiện** (standard 3 Pillars):

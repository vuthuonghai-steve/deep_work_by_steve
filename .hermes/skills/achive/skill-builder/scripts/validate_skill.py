...
class SkillValidator:
    ...
    def check_skill_md_constraints(self):
        ...
        mandatory_keywords = ["## Persona", "## Mission", "## Workflow", "Guardrails"]
        found_missing = []
        for kw in mandatory_keywords:
            if kw not in content:
                found_missing.append(kw)
        # Persona may be defined in YAML frontmatter or as ## Mission/## Overview
        # Only fail if NONE of the persona-related keywords are found
        persona_related = ["## Persona", "## Mission", "## Overview", "## Summary"]
        has_persona = any(kw in content for kw in persona_related)
        if found_missing == mandatory_keywords and not has_persona:
            self.errors.append(f"[E04] ERROR: SKILL.md missing mandatory section keyword: one of {mandatory_keywords}")
    ...
    def check_file_mapping(self):
        ...
        # FIX: Only extract paths from "Files cần tạo" column (2nd column in table)
        lines = design_content.split('\n')
        in_zone_mapping = False
        for line in lines:
            if '## 3. Zone Mapping' in line or '## 3. ' in line:
            ...
            if in_zone_mapping:
                ...
                # Files cần tạo is typically column index 1 or 2 depending on format
                # Look for cells containing backtick-enclosed paths
                for cell in cells:
                    if '`' in cell and ('/' in cell or cell == 'SKILL.md'):
                        # Extract backtick-enclosed content from this cell
                        cell_matches = re.findall(r"`([^`]+)`", cell)
                        for m in cell_matches:
                            m = m.strip()
                            if m and (m == 'SKILL.md' or '/' in m):
        ...
        ignore_extra = {
            ...
            "templates/cleaned-prompt.xml.template",
            # Python bytecode cache directories
            "__pycache__",
            "scripts/__pycache__",
            "spec-generator-has-api/scripts/__pycache__",
        }

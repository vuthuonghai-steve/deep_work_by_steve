...
# sandbox-validator — Phân Rã Kiến Trúc Micro-Skill
...
- **Tác vụ**: Thực thi kiểm định an toàn mã nguồn và chạy các schema validator trong Docker Sandbox cô lập (gVisor).

### B. Vai trò trong Orchestration Flow
[TỪ EXPLORATION §5]
Quy hoạch khuyến nghị phân vùng Zones ban đầu: Core (SKILL.md), Scripts (scripts/validate.py), Data (data/validated_artifacts.yaml), Loop (loop/checklist.md)

## 2. Capability Map

[TỪ EXPLORATION §3.3 & §4]
- **Nhiệm vụ nghiệp vụ chính**: Thực thi kiểm định an toàn mã nguồn và chạy các schema validator trong Docker Sandbox cô lập (gVisor).

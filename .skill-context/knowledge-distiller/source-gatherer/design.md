...
- **Tác vụ**: Thu thập, quét tài nguyên từ codebase/web, lọc nhiễu và bọc XML boundaries an toàn cho dữ liệu thô chống Prompt Injection.

### B. Vai trò trong Orchestration Flow
[TỪ EXPLORATION §5]
Quy hoạch khuyến nghị phân vùng Zones ban đầu: Core (SKILL.md), Scripts (scripts/gather.py), Data (data/raw_source.xml), Loop (loop/checklist.md)

## 2. Capability Map

[TỪ EXPLORATION §3.3 & §4]
- **Nhiệm vụ nghiệp vụ chính**: Thu thập, quét tài nguyên từ codebase/web, lọc nhiễu và bọc XML boundaries an toàn cho dữ liệu thô chống Prompt Injection.

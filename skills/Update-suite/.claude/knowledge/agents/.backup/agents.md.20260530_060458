...
Read-only agent for research during planning mode.

### General-purpose

Handles complex multi-step changes and research with full tool access.

### Other

Claude Code also includes a `claude-code-guide` subagent for answering questions about Claude Code itself.
...
To restrict tools, use either the tools field (allowlist) or the disallowedTools field (denylist). This example uses tools to exclusively allow Read, Grep, Glob, and Bash. The subagent can't edit files, write files, or any MCP tools:
...
1. Claude spawns a fork whenever it would otherwise use the general-purpose subagent. Named subagents such as Explore still spawn as before.
2. Every subagent spawn runs in the background, whether it is a fork or a named subagent. Set CLAUDE_CODE_DISABLE_BACKGROUND_TASKS to 1 to keep spawns synchronous.
...
if echo "$COMMAND" | grep -iE '(INSERT|UPDATE|DELETE|DROP|CREATE|ALTER|TRUNCATE|REPLACE|MERGE)' > /dev/null; then
